...
class VideoStreamServer: NSObject, ObservableObject {
    ...
    private func stopServerImmediate() {
        ...
        // Stop all connection timers immediately on main thread
        DispatchQueue.main.async { [weak self] in
            self?.connections.forEach { wsConn in
                wsConn.invalidate()
            }
        }
    ...
    private func startStreamingToConnection(_ wsConn: WebSocketConnection) {
        ...
        DispatchQueue.main.async { [weak self, weak wsConn] in
            guard let self = self, let wsConn = wsConn, self.isRunning else { return }
            
            let timer = Timer.scheduledTimer(withTimeInterval: 1.0/30.0, repeats: true) { [weak self, weak wsConn] timer in
                guard let self = self, let wsConn = wsConn else {
                    timer.invalidate()
                    return
                }
    ...
    private func removeConnection(_ wsConn: WebSocketConnection) {
        queue.async { [weak self] in
            ...
            if let index = self.connections.firstIndex(where: { $0.connection === wsConn.connection }) {
                self.connections[index].invalidate()
